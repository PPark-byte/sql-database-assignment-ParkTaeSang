...
select s.sales_dept_id, sd.sales_dept_name, sd.sales_region,
count(*) as transaction_count,
sum(s.quantity) as total_quantity,
sum(s.sale_amount) as total_sales_amount,
round(avg(s.sale_amount),2) as avg_sales_amount
from public.sales as s
join public.sales_departments as sd
on s.sales_dept_id = sd.sales_dept_id
where s.sale_date between '2025-04-01' and '2025-06-30'
and s.sales_type != '반품'
group by s.sales_dept_id,sd.sales_dept_name, sd.sales_region
order by total_sales_amount desc;
...
1.
총 판매 금액 1위 제품 : P004: 리피다운정 20mg, 17,145,700원
총 판매 금액 1위 부서 : D04 : 호남영업팀, 34,490,967원

2.
그룹 분류 기준
상위 그룹: 총 판매 금액 30,000,000원 이상
중위 그룹: 총 판매 금액 20,000,000원 ~ 30,000,000원 미만
하위 그룹: 총 판매 금액 20,000,000원 미만


 3.
 수도권병원2팀의 경우 평균 판매 금액은 양호하나 거래 건수가 최하위여서 총 판매 금액이 적었다.
 강원영업팀의 경우 거래 건수는 수도권병원2팀보다 많았으나,
 평균 판매 금액이 호남영업팀의 절반 이하여서 최하위 실적을 기록했다.
